...
class CartControllerTest extends SecurityContextMocking {
    ...
    void shouldReturnCartContent() throws Exception {
        final UserDetails userDetails = new UserDetailsImpl(new User("adamsmith@yahoo.com", "12345", Role.USER));
        ...
        when(cartService.getCartItems("adamsmith@yahoo.com")).thenReturn(expectedDto);

        final MvcResult mvcResult =
                mockMvc.perform(get("/user/cart").with(user(userDetails))
                                .contentType(APPLICATION_JSON))
                        .andExpect(status().isOk())
                        .andReturn();
        ...
        verify(cartService).getCartItems("adamsmith@yahoo.com");

        assertThat(actualResponseBody).isEqualToIgnoringWhitespace(objectMapper.writeValueAsString(expectedDto));

    }

    @Test
    void shouldAddToCartListOfItem() throws Exception {

        final CartItemDto cartItemDto1 = new CartItemDto(new ProductDtoShort(1L, "image1", "name1", 50, 4,
                Type.WEIGHABLE, 1), 5);
        final List<CartItemDto> cartItemDtoList = List.of(cartItemDto1);
        final CartDto expectedDto = new CartDto(cartItemDtoList, 250);

        final MvcResult mvcResult = mockMvc.perform(post("/user/cart/add")
                        .with(user("adamsmith@yahoo.com"))
                        .contentType(APPLICATION_JSON)
        ...
        verify(cartService).addItemsToCard(expectedDto, "adamsmith@yahoo.com");
